...
Protoplus.ui = {
  ...
  tooltip: function (element, text, options) {
    ...
    text = options.title
      ? "<strong>" + options.title + "</strong><br>" + text
      : text;
    element.hover(
      function (el, evt) {
  ...
  slider: function (element, options) {
    ...
    var sliderWheel = function (e) {
      ...
      var w = Event.wheel(e);
      if (w > 0) {
        moveRIGTH(5); // If scroll up then move to the right
      } else if (w < 0) {
        moveLEFT(5);
