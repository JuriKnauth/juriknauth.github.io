      ...
      _isButton = function (event, code) {
        switch (code) {
          ...
          case 1:
            return event.which == 1 && event.metaKey;
          case 2:
            return event.which == 3 && !event.metaKey; // Safari and chrome right click fix
          default:
            return false;
        }
      };
...
Protoplus.utils = {
  ...
  shift: function (element, options) {
        ...
        if (element.tagName == "BODY") {
          from =
            okey == "scrollX" || okey == "scrollLeft"
              ? window.scrollX
              : window.scrollY; // Read the window scroll
        } else {
          from =
            okey == "scrollX" || okey == "scrollLeft"
              ? element.scrollLeft
    ...
    var step = function () {
      ...
      if (time >= end) {
        // If duration is done. Complete the animation
        clearInterval(element.timer);
        element.timer = false;
        ...
        for (okey in properties) {
          ...
          if (["scrollX", "scrollLeft", "scrollY", "scrollTop"].include(okey)) {
            if (element.tagName.toUpperCase() == "BODY") {
              // In order to scroll the document
...
Ajax = Object.extend(Ajax, {
  ...
  Jsonp: function (url, options) {
    ...
    if (!sameDomain && this.options.force === false) {
      // If url is not external then convert it to Ajax.Request
    ...
    this.url =
      ...
      callback_id +
      "&nocache=" +
      new Date().getTime(); // In order to prevent cacheing
