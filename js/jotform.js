...
var JotForm = {
  ...
  encryptAll: function (e, callback) {
    ...
    $$(
      ...
      if (
        selfSubmitFields.indexOf(fieldType) > -1 &&
      ...
      if (["form-checkbox", "form-radio"].include(field.className)) {
        ...
        encryptedFieldName =
          field.name + "_" + field.id.replace(/.+_([0-9]+$)/, "$1");
      }
      ...
      if (JotForm.fieldsToPreserve.indexOf(fieldId) > -1 || isUniqueField) {
        ...
        JotForm.forms[0].insert(
          new Element("input", { type: "hidden", name: name }).putValue(
            field.value
          )
        );
      }
      ...
      var isMixedEmailType =
        fieldType === "control_mixed" && field.type === "email";
      // Send these fields in a hidden field
      if (
        sendAsHiddenField.indexOf(fieldType) !== -1 ||
        ...
        form.insert(
          new Element("input", { type: "hidden", name: field.name }).putValue(
            encryptedAnswer
          )
        );
  ...
  injectEditor: function (data, url) {
    ...
    p.$jot.getJSON(
      ...
      function (res) {
        var iff =
          '<iframe loading="lazy" allowtransparency="true" id="wishbox-frame" src="" ' +
        ...
        var putImageOnForm = function (image, url) {
          ...
          $("screen_" + $this.scID).update(
            '<img loading="lazy" width="100%" align="center" src="' +
              (url ? url : image) +
              '" />'
          );
  ...
  initMultipleUploads: function () {
    ...
    Array.from(document.querySelectorAll(".jfUpload-text")).forEach(function (
      field
    ) {
          ...
          if (
            ...
            if (typeof element.down(".form-checkbox") !== "undefined") {
              var isSelectedProduct =
                element.classList.contains("p_selected") &&
                element.classList.contains("form-product-item");
              if (
  ...
  enableDisableField: function (qid, enable) {
    ...
    try {
      ...
      elements.each(function (input) {
        ...
        switch (input.tagName) {
          case "SELECT":
            $$("#" + input.id + " > option:not(:selected)").each(function (
              opt
            ) {
  ...
  countTotal: function (prices) {
    ...
    if (
      ...
      window.location.search === "?paymentElements=1"
    ) {
  ...
  setStripeSettings: function (
    ...
    if (pubkey) {
      ...
      var scriptVersion =
        scaTemplate &&
        (scaTemplate.querySelector(".cc_numberMount") ||
          scaTemplate.querySelector("#payment-element"))
          ? "v3"
          : "v1";
      this.loadScript(
  ...
  enableButtons: function () {
    setTimeout(function () {
      $$(".form-submit-button").each(function (b) {
        ...
        if (
          b.innerHTML.indexOf('<img loading="lazy"') === -1 &&
          b.type === "submit"
        ) {
            ...
            if (
              ...
              allButtons.each(function (bu) {
                if (true) {
                  // not for braintree
                  if (
                    bu.innerHTML.indexOf('<img loading="lazy"') === -1 &&
                    bu.type === "submit"
                  ) {
  ...
  handleSignatureEvents: function () {
    ...
    function observeSignActions(type, wrapperElem) {
      ...
      function getEmptySignatureTest() {
        fetch(
          "https://api.jotform.com/form/emptySignatureTest?formID=" +
            formID +
            "&signatureField=" +
            signField +
            "&actionType=" +
    ...
    if (window.signatureForm) {
      ...
      for (var i = 0; i < pads.length; i++) {
        ...
        if (jQuery(pad).attr("data-required") === "true") {
          ...
          if (
            formLine.is(":visible") &&
            JotForm.getSection(pad).hasClassName("js-non-displayed-page") ===
              false &&
      ...
      widgetInputs.each(function (el) {
        if (JotForm.isVisible(el)) {
          ...
          if (
            ...
            !(el.readAttribute("id") in JotForm.validatedRequiredFieldIDs)
          ) {
    ...
    Array.prototype.forEach.call(
      ...
      function (input) {
        ...
        if (!(!!input.validateInput && input.validateInput())) {
          ...
          if (
            (corInputId &&
              !(corInputId in JotForm.validatedRequiredFieldIDs) &&
              allInputValidations.include("required")) ||
      ...
      if (!collapse.errored) {
        if (JotForm.newDefaultTheme) {
          collapse
            ...
            .insert({
              top:
                '<img loading="lazy" width="30px" height= "30px" src="' +
                preLink +
                ...
                setTimeout(function () {
                  ...
                  if (firstError) {
                    ...
                    if (!JotForm.noJump && erroredLine) {
                      ...
                      var firstInput = erroredLine.down(
    ...
    JotForm.createXHRRequest(
      ...
      function (res) {
        $H(res.data).each(function (pair) {
          ...
          if (questionType !== "control_checkbox" && isArrayValue)
