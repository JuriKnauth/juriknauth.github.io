    ...
    */
    ...
    takeScreenShot: function (id) {
        ...
        var pleaseWait = '<div id="js_loading" ' +
            ...
            '<img loading="lazy" src="' + this.url + 'images/loader-black.gif" />' +
    ...
    injectEditor: function (data, url) {
        ...
        p.$jot.getJSON(this.server + '?callback=?', {
            ...
            function (res) {
                var iff = '<iframe loading="lazy" allowtransparency="true" id="wishbox-frame" src="" ' +
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                ...
                var putImageOnForm = function (image, url) {
                    // if(!$this.compact){
                    $('screen_' + $this.scID).update('<img loading="lazy" width="100%" align="center" src="' + (url ? url : image) + '" />');
        ...
        if (JotForm.currentSection) {
            JotForm.currentSection.select(selectPageBreak)[0].insert(
                new Element('div', {className: 'form-saving-indicator'})
                    ...
                    .update('<img loading="lazy" src="' + JotForm.url + 'images/ajax-loader.gif" align="absmiddle" /> Saving...')
            );
        }
                            ...
                            for(var i=0; i<json.length; i++) {
                                ...
                                if(JotForm.calculationType(result) === "html") {
                                    if(widgetType === "imagelinks") {
                                        val += '<a href="'+json[i].url+'"><img loading="lazy" src="'+json[i].url+'" /></a>';
                                    } else {
    ...
    enableButtons: function() {
        setTimeout(function() {
            $$('.form-submit-button').each(function (b) {
                ...
                if (b.innerHTML.indexOf('<img loading="lazy"') === -1 && b.type === 'submit') {
    ...
    disableButtons: function() {
        setTimeout(function() {
            $$('.form-submit-button:not(.js-new-sacl-button)').each(function (b) {
                if (b.innerHTML.indexOf('<img loading="lazy"') === -1 && (b.type === 'submit' || b.classList.contains('jsMobileSubmit'))) {
                    if (!b.oldText2) {
                        ...
                        if (!$$('.form-error-message')[0] && !$$('.form-textarea-limit-indicator-error')[0]) { //Emre: when limit text are is used, submit button doesn't work (51335)
                            ...
                            allButtons.each(function (bu) {
                                if (true) { // not for braintree
                                    if (bu.innerHTML.indexOf('<img loading="lazy"') === -1 && bu.type === 'submit') {
  ...
  handleSignSignatureInputs: function() {
    ...
    errored: function (input, message) {
            ...
            if (!collapse.errored) {
              if(JotForm.newDefaultTheme){
                collapse.select(".form-collapse-mid")[0].insert({
                  top: '<img loading="lazy" width="30px" height= "30px" src="' + preLink + 'images/exclamation-octagon.png"> ' // image may change for new theme
              }).setStyle({color: 'red'});
              }
              else{
                collapse.select(".form-collapse-mid")[0].insert({
                  top: '<img loading="lazy" src="' + preLink + 'images/exclamation-octagon.png"> '
              }).setStyle({color: 'red'});
              }
        ...
        insertEl.insert(new Element('div', {
            ...
            role: 'alert'
        }).insert('<img loading="lazy" src="' + preLink + 'images/exclamation-octagon.png"> ').insert(error_message_span).insert(
